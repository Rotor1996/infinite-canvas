...
+ [新增] 默认渠道 API Base URL 与 API Key 获取入口支持通过构建期或容器运行时环境变量配置。

## v0.11.1 - 2026-07-29

+ [修复] Grok 视频编辑与延展在提交前校验提示词非空，避免向上游发送无效请求。
...
+ [修复] 修复 Sub2API Grok 视频完成后返回相对内容地址时，前端请求到错误域名且未携带 API Key，导致视频无法保存和播放的问题。
+ [修复] 修复新建分组节点后未自动打开节点设置面板的问题。
+ [修复] 修复 Grok 参考视频使用站内代理相对路径时无法进入生成请求的问题。
+ [修复] 修复 Grok 视频节点重试时重新组装错误提示词和丢失素材引用的问题。
+ [修复] 修复画布视频重试时可能沿用旧图片模型，导致 Grok 参考视频被误判为普通视频接口的问题。
+ [新增] Grok / xAI 远程视频结果支持通过站点同源媒体代理播放，避免用户浏览器直连 `vidgen.x.ai` 超时。
+ [新增] Grok 视频生成支持参考视频编辑与末帧延展模式。
+ [新增] 视频生成支持通过 Sub2API / xAI 的 Grok 异步视频协议创建和轮询任务，并适配 Grok 比例、分辨率、时长及图片输入。
+ [新增] 模型渠道增加明确的 Grok / xAI 调用格式，使用独立默认地址并驱动 Grok 视频协议分流。
+ [新增] Vite 本地开发支持通过环境变量启用 Sub2API 同源反向代理，绕过开发环境浏览器 CORS 限制。
+ [调整] 默认将 OpenAI 与 Grok 模型拆分到独立渠道，并明确 Sub2API 两类分组 Key 需要分别配置。
+ [调整] Docker Compose 默认把文档站挂载到主应用同域 `/doc` 路径。
+ [调整] 隐藏首页导航栏中的文档和 GitHub 入口。
