...
const generationOptionsSchema = z.object({
    ...
    videoReferenceMode: z.enum(["edit", "extend"]).optional(),
...
export const toolInputSchemas = {
    ...
    workbench_video_generate: z.object({ prompt: z.string(), model: z.string().optional(), size: z.string().optional(), seconds: z.string().optional(), referenceMode: z.enum(["edit", "extend"]).optional(), resolution: z.string().optional(), generateAudio: z.boolean().optional(), watermark: z.boolean().optional(), run: z.boolean().optional() }),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    workbench_video_get_config: "读取视频创作台的当前参数和可选项（可用模型、尺寸/比例、时长、Grok 参考视频模式、清晰度/分辨率、是否生成声音与水印）。",
    workbench_video_generate: "在视频创作台填入提示词并按需设置 model、size、seconds、referenceMode（Grok 参考视频模式：edit 编辑 / extend 延展）、resolution、generateAudio、watermark，run 默认 true 会自动点击生成按钮。会自动跳转到视频创作台。生成为异步过程，提交后返回 taskId，可用 generation_get_status 查询状态。",
