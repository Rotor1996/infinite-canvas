...
- 排查 AI 错图、重复图或提示词不一致时，必须比较上游响应中解码后的图片字节哈希与客户端实际保存文件的哈希；仅凭访问日志、响应 URL、节点引用或视觉相似不能完成归因。
- Docker entrypoint 和服务器 Shell 脚本必须通过 `.gitattributes` 固定为 LF；发布前需检查 Git 归档中的脚本不包含 CRLF，避免容器把解释器识别为 `/bin/sh\r`。
