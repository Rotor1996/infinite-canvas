...
- Grok 视频生成：分别配置 OpenAI 格式和 Grok / xAI 格式的两条 Sub2API 渠道，使用各自分组 Key（Base URL 可相同，模型分别归属对应渠道）；选择 `grok-imagine-video` 或 `grok-imagine-video-1.5` 后，需验证文生视频、单图生视频、基础模型多图参考、单参考视频编辑、单参考视频末帧延展、任务轮询和生成结果本地保存；同时验证 OpenAI 模型仍使用 OpenAI 渠道 Key，1.5 多图参考、非单图 1080p、多图超过 10 秒、Grok 同时连接参考图和参考视频、多个参考视频、参考音频会显示明确提示。
