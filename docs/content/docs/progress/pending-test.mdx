...
- 图片编辑结果：当接口返回无法由浏览器跨域下载的临时图片 URL 时，画布应直接展示远程图片而不是报 `Failed to fetch`；同时需确认可返回 base64 或允许 CORS 的图片仍会保存到浏览器本地。
- 首页导航栏：隐藏文档和 GitHub 入口，需验证主应用首页顶部栏和文档站首页导航不再展示对应入口。
- Vite 本地代理：设置 `VITE_SUB2API_PROXY_TARGET` 后，将渠道 Base URL 配置为 `http://localhost:3000/sub2api`，需验证模型列表、图片生成和 Grok 视频请求均通过本地同源地址转发，且生产构建不依赖该代理。
- Grok 视频生成：分别配置 OpenAI 格式和 Grok / xAI 格式的两条 Sub2API 渠道，使用各自分组 Key（Base URL 可相同，模型分别归属对应渠道）；选择 `grok-imagine-video` 或 `grok-imagine-video-1.5` 后，需验证文生视频、单图生视频、基础模型多图参考、单参考视频编辑、单参考视频末帧延展、任务轮询和生成结果本地保存；同时验证生成配置节点从生图切换到视频后会使用 Grok 模型，组装提示词时只有通过 `@` 标签引用的连接素材进入请求；OpenAI 模型仍使用 OpenAI 渠道 Key，1.5 多图参考、非单图 1080p、多图超过 10 秒、Grok 同时连接参考图和参考视频、多个参考视频、参考音频会显示明确提示。
