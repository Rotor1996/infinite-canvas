...
- Grok 视频生成：分别配置使用 OpenAI 分组 Key 和 Grok 分组 Key 的两条 Sub2API 渠道（Base URL 可相同，模型分别归属对应渠道），选择 `grok-imagine-video` 或 `grok-imagine-video-1.5` 后，需验证文生视频、单图生视频、基础模型多图参考、任务轮询和生成结果本地保存；同时验证 OpenAI 模型仍使用 OpenAI 渠道 Key，1.5 多图参考、非单图 1080p 和多图超过 10 秒会显示明确提示。
